...
namespace Util_.JsonHelper_
{
	...
	public abstract class BaseJsonConverter<T> : JsonConverter
	{
		...
		{
			BindingFlags _flag = BindingFlags.Instance | BindingFlags.Public;

			FieldInfo _field = typeof(T).GetField(name, _flag);
			if (null != _field)
			{
				return _field;
			}

			PropertyInfo _property = typeof(T).GetProperty(name, _flag);
			if (null == _property)
			{
				Throw(name, "Public instance field or property {0} is not found.");
			}

			if (null == _property.GetGetMethod())
			{
				Throw(name, "Property {0} is not readable.");
			}
			if (null == _property.GetSetMethod())
			{
				Throw(name, "Property {0} is not writable.");
			}
